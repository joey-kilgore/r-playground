...
mhCalcDF <- function(tempm, temph){
  ...
  for (num in 2:(ncol(tempm) - 1)) {
    # loop through every column of the data (which is every node)
    tempmhDF <- cbind(tempmhDF, NODE = tempm[1:nrow(tempm), num] * temph[1:nrow(temph), num]) # add the next column of the data to the data frame
